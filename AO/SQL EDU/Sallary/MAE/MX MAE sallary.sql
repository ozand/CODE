...
(case  when  extract(year from sm.closed_at) in ('2015')  then '1' else 0 end) as seminar_closed, sm.city_id, sm.name,
...
where extract(year from started_at) in ('2015') --and sm.id in ('116973')
