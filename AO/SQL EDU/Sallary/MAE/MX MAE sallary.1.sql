select  SM.id , 
sum(smu.paid) as Paid_sem, 
sm.seminar_type_id, 
sm.studio_id, 
sm.technolog_id, 
extract(day from sm.started_at) as Day, 
extract(month from sm.started_at)as Month, 
extract(year from sm.started_at) as Year,
to_char(sm.started_at,'dd.mm.YYYY') as FullDate ,

(case  when  extract(year from sm.closed_at) in ('2016')  then '1' else 0 end) as seminar_closed, 
sm.city_id, 
smt.name,
...
, (select sln.com_sect from salons as sln where sm.salon_id = sln.id) as sec 
, (select sln.com_reg from salons as sln where sm.salon_id = sln.id) as reg 

from seminars as SM
left join seminar_users as SMU ON sm.id = smu.seminar_id
left join seminar_types as smt on sm.seminar_type_id = smt.id
where extract(year from started_at) in ('2016')


GROUP BY sm.id, smt.name