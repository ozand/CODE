select concat(sm.technolog_id, sm.partimer_id) as idUser, sm.seminar_type_id, to_char(started_at,'DD') as Day, 
...
--(case  when sm.trip = 't' then '1' else 0 end) as trip,
(case  when to_char (sm.closed_at, 'YYYY') in ('2015')  then '1' else 0 end) as seminar_closed,
concat( sm.technolog_full_name, sm.partimer_full_name) as Name,
