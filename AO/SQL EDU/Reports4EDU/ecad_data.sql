...
trim (concat(sln.name, '. ', SLN.address, '. ', sln.city_name_geographic)) as "salon_name", 
sln.city_name, 
...
(case when spp.name like '%Expert%' or spp.name like '%МБК%' then spp.status end) as Club,
(case when spp.name like '%Emotion%' then spp.status end) as eMotion
...
left join 
	...
	ON
	sln.id = spp.salon_id and spp.brand_id = 1 
	-- LP-1:ES-3:MX-5:KR-6:RD-7


GROUP BY sln.id , spp.name, spp.status