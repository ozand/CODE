...
(Case when slnMNG.id is not null then 'salon_manager' else 
(Case when usr.id is not null then 'hairdresser' else 'not_reg_users' end) end) end) as type_master,
...
where spc.brand_id = 1')
AS spp(status  text, id integer, name name, brand_id  integer, salon_id  integer )
ON
